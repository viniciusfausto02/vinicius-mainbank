...
export default function InsightsPanel() {
  ...
  // Hooks must be called unconditionally to keep order stable across renders
      ...
      <div className="rounded-xl border border-slate-800/50 bg-slate-950/40 p-3">
        <div className="mb-2 flex items-center justify-between">
          ...
          <label className="flex items-center gap-2 text-xs text-slate-300 select-none">
            <input type="checkbox" checked={showTrend} onChange={(e) => setShowTrend(e.target.checked)} className="h-4 w-4 rounded border-slate-600 bg-slate-800" />
            {t("insightsShowTrend")}
          </label>
        </div>
